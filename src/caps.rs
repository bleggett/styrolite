...
mod tests {
    ...
    fn test_cap_encoding_word_order() {
        ...
        let input = CapResult { effective: low_caps, permitted: low_caps, inheritable: low_caps };
        let data = cap_result_to_data(&input);
        assert_eq!(data[0].effective, low_caps as u32, "data[0] must hold low 32 bits (caps 0-31)");
        assert_eq!(data[1].effective, 0u32, "data[1] must be zero for low-only caps");
        ...
        let input = CapResult { effective: high_caps, permitted: high_caps, inheritable: high_caps };
        let data = cap_result_to_data(&input);
        assert_eq!(data[0].effective, 0u32, "data[0] must be zero for high-only caps");
        assert_eq!(data[1].effective, (high_caps >> 32) as u32, "data[1] must hold high 32 bits (caps 32-63)");
        let rt = cap_data_to_result(&data);
        assert_eq!(rt.effective, high_caps);

        // All caps: bits 0-40
        let all_caps = CapabilityBit::raw_bits(CapabilityBit::ALL);
        let input = CapResult { effective: all_caps, permitted: all_caps, inheritable: all_caps };
        let data = cap_result_to_data(&input);
